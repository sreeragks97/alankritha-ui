...
export default function MediaLibraryPage() {
  ...
  useEffect(() => {
    void adminRepository.getMediaAssets().then(setAssets);
  }, []);
    ...
    <div className="space-y-4">
      <section className="card-luxury rounded-2xl p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-2">
            <select className="rounded-xl border border-[#e8dcc3] bg-white px-3 py-2 text-sm">
              <option>Filter by type</option>
              <option>Image</option>
            </select>
            <button type="button" className="rounded-xl bg-[var(--brand-gold)] px-4 py-2 text-sm font-semibold text-white">
              Upload Media
            </button>
          </div>
        </div>
      ...
      <section className="card-luxury rounded-2xl p-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filtered.map((asset) => (
            <article key={asset.id} className="rounded-xl border border-[#ebdfca] bg-white p-3">
              <button
                type="button"
