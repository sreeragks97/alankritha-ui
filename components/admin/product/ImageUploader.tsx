  ...
  return (
    <div className="space-y-4">
      <button
        ...
        className="flex w-full flex-col items-center justify-center rounded-2xl border border-dashed border-[#d7c49a] bg-[#fffaf0] px-4 py-9 text-center"
      >
      ...
      {images.length > 0 ? (
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {images.map((image, index) => (
            <div key={`${image}-${index}`} className="rounded-xl border border-[#ebdfca] bg-white p-3">
              <div className="aspect-[4/3] rounded-lg bg-cover bg-center" style={{ backgroundImage: `url(${image})` }} />
              <div className="mt-3 flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={() => moveImage(index, -1)}
                  className="rounded-md border border-[#e6d7bb] px-2 py-1 text-xs"
                >
                  Move Left
                </button>
                <button
